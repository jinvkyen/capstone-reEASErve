...
<body class="bg-white font-mont">
    <div class="flex flex-col lg:flex-row">
        ...
        <!-- Main Content -->
        <div class="flex flex-col flex-1 p-4 lg:ml-80">
            <!-- Title -->
            <div class="header flex justify-between ml-4 lg:ml-8">
                <h1 class="mt-5 text-4xl font-bold">Audit Trail</h1>
            </div>

            <!-- Search and Table Container -->
            <div
                id="audit"
                class="tab-pane mx-2 p-4 mt-6 font-mont rounded-lg shadow-lg sm:text-sm"
                ...
                <div class="flex justify-between">
                    ...
                    <div class="flex flex-row mt-2 lg:mt-0 space-x-2">
                        ...
                        <!-- Archive Past 7 Days Button -->
                        <form action="{{ route('system.audit.mass.archive') }}" method="POST" id="delete7daysForm">
                            @csrf
                            <input type="hidden" name="submit_type" value="form">
                            <div class="relative flex items-center group">
                                <button type="submit" class="bg-purple-500 text-white lg:px-4 lg:py-2.5 sm:py-1 sm:px-2 rounded-md hover:bg-purple-600 transition flex items-center">
                ...
                <div class="flex lg:sm:flex-row md:flex-col sm:flex-col lg:justify-end md:justify-end sm:justify-center m-0 py-2 px-2">
                    ...
                    <select id="statusFilter" class="leading-tight mt-1 flex-row w-18 border-gray-300 rounded-md p-1 font-mont overflow-y-auto">
                        ...
                        @foreach($combinedRequestsPaginated->unique('department') as $status)
                        <option value="{{ $status->department }}">
                            {{ $status->department }}
                        </option>
                        @endforeach
                    </select>

                    <label for="tableFilterDropdown" class="flex-col text-sm font-medium text-gray-700 px-2 mt-3">
                        <i class="fas fa-filter text-gray-500 text-lg"></i> Table
                    </label>
                ...
                <div class="lg:max-h-[29rem] sm:max-h-screen overflow-x-auto mt-4" id="auditTableContainer">
                    ...
                    <table id="auditTable" class="min-w-full table-fixed">
                        <thead
                            class="bg-gray-100 uppercase font-large text-xs top-0 sticky z-10">
                        ...
                        <tbody>
                            ...
                            <tr class="table-row bg-white border-b hover:bg-gray-50">
                                <td class="w-32 px-2 lg:px-4 py-2 font-medium text-gray-900 whitespace-wrap break-words text-center">
                                    {{ $entry->id }}
                                </td>
                                <td class="px-2 lg:px-4 py-2 whitespace-wrap text-sm text-gray-500">
                                    <div class="status-cell text-sm font-medium text-gray-700">
                                        <span class="status-label">{{ $entry->department }}</span>
                                    </div>
                                </td>
                                <td class="px-2 lg:px-2 py-2 text-sm text-gray-500 whitespace-nowrap">
                                ...
                                <td class="px-2 lg:px-4 py-2 max-w-xs font-medium text-gray-900 whitespace-wrap group relative text-balance">
                                ...
                                <td class="px-2 lg:px-4 py-2 font-medium text-gray-900 whitespace-wrap">
                                ...
                                <td class="px-2 lg:px-4 py-2 no-underline text-center">
                                    <div class="flex flex-row relative items-center group">
                                        <div class="flex flex-col relative items-center group">
                                            ...
                                            <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 hidden group-hover:block bg-white text-xs mb-2 rounded px-2 py-2 whitespace-nowrap shadow-lg z-20">
                                                Restore Records
                                            </div>
                                        </div>
                                    </div>
...
<script>
    document.getElementById("searchInput").addEventListener("keyup", function() {
        ...
        const route = "{{ route('system.audit.trail') }}";
...
<script>
    $(document).ready(function() {
        ...
        $(document).on("click", ".deleteModalTrigger", function() {
            ...
            $.ajax({
                url: '{{ route("system.audit.archive", ["id" => "__id__"]) }}'.replace(
                    "__id__",
                    id
                ),
                type: "POST",
...
<script>
    $(document).ready(function() {
        ...
        $('.restoreModalTrigger').on('click', function() {
            ...
            $.ajax({
                url: '{{ route("system.audit.restore", ["id" => "__id__"]) }}'.replace('__id__', id),
                type: 'POST',
                data: {
                    _method: 'POST',
                    _token: '{{ csrf_token() }}',
