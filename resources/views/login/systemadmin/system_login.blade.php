    ...
    <div class="w-full h-full flex">
        ...
        <div class="bg-gradient-to-t from-slate-50 to-[#BEE9F0] md:w-2/5 hidden md:flex flex-col justify-between">
            ...
            <div class="flex flex-grow items-end justify-center">
                <img src="{{ asset('storage/assets/system admin image.png') }}" id="login-image" alt="Login Image" class="h-81 w-auto opacity-0 transition-all duration-500 ease-in-out transform" draggable="false" oncontextmenu="return false;">
            </div>
        </div>

        <!-- Right Container (Login Form) -->
        <div class="responsive-bg bg-white md:shadow-xl w-full md:w-3/5 flex items-center justify-center p-8 relative">
            <div class="responsive-container w-full mx-5">
                <div class="mb-12">
                    <!-- Logo -->
                    <img src="{{ asset('storage/assets/logo.png') }}" id="logo" alt="Logo" class="md:hidden absolute top-8 left-6 h-14 w-auto mb-6 opacity-0 transition-all duration-500 ease-in-out transform">
                    <h1 class="text-4xl md:text-6xl font-bold mb-1">Welcome Back</h1>
                    <p class="text-xl md:text-2lg text-200">Login as System Admin</p>
                </div>
                <form method="POST" action="{{route('system.logged')}}" id="login-form">
                    @csrf
                    <!-- Email -->
                    <div class="mb-4">
                        <input id="login-field" type="text" name="email" placeholder="Email" value="{{ session('email') ?? Cookie::get('remember_system_email') }}" class="w-full p-3 border border-gray-300 rounded-lg">
                        @error('email')
                    ...
                    <div class="mb-4">
                        <div class="relative">
                            <input id="password" type="password" class="w-full p-3 border border-gray-300 rounded-lg" name="password" placeholder="Password">
                    ...
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <a href="{{route('system.forgotpass')}}" class="text-[#385F6E] md:underline-offset-4 hover:underline">Forgot Password?</a>
                    </div>
