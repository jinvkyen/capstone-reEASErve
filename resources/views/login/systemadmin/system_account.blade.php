            ...
            <div class="flex flex-col p-2 rounded-lg shadow-lg mx-4 mt-8 font-mont" style="background-color: var(--custom-color);">
                <div class="flex flex-col lg:flex-row items-center align-middle lg:p-10 sm:p-2">
                    <div class="lg:order-1 m-3 lg:pr-4">
                        <img class="w-32 h-32 sm:w-36 sm:h-36 lg:w-48 lg:h-48 rounded-full border border-gray-500" src="{{asset('storage/assets/default_image.png')}}" alt="User Image">
                    </div>

                    <div class="lg:order-2 flex flex-col items-center lg:items-start text-center lg:text-left w-full lg:w-1/3 space-y-2">
                        <p class="sm:text-sm lg:text-xl font-bold mb-0"></p>
                        <p class="sm:text-sm lg:text-lg text-gray-500 mt-0 mb-4"></p>
                        <div class="overflow-x-auto w-full">
                            <table class="sm:text-xs lg:text-base text-left w-full text-balance">
                                <tr>
                                    <td class="pb-1 font-bold">{{ $userData['first_name'] }} {{ $userData['last_name'] }}</td>
                                </tr>
                                <tr>
                                    <td class="pb-3">{{ $userData['email'] }}</td>
                                </tr>
                            </table>

                            <form method="POST" action="{{ route('system.credentials.change') }}" id="change-credentials-form">
                                @csrf
                                <!-- First Name -->
                                <div class="mb-2">
                                    <input id="firstname" type="text" name="firstname" value="{{ old('firstname', auth()->user()->first_name) }}" placeholder="First Name" class="w-full p-1 border border-gray-300 rounded-lg text-sm font-mont">
                                </div>

                                <!-- Last Name -->
                                <div class="mb-2">
                                    <div class="relative">
                                        <input id="lastname" type="text" name="lastname" value="{{ old('lastname', auth()->user()->last_name) }}" placeholder="Last Name" class="w-full p-1 border border-gray-300 rounded-lg text-sm font-mont">
                                    </div>
                                </div>

                                <!-- Email -->
                                <div class="mb-2">
                                    <input id="email" type="text" name="email" value="{{ old('email', auth()->user()->email) }}" placeholder="Email" class="w-full p-1 border border-gray-300 rounded-lg text-sm font-mont">
                                </div>

                                <div class="flex justify-left">
                                    <button type="submit" class="w-3/5 p-1 bg-blue-500 hover:bg-blue-600 text-white rounded-lg mt-3 mb-8 font-mont sm:text-xs lg:text-sm">
                                        SAVE
                                    </button>
                                </div>
                            </form>

                            <table class="sm:text-xs lg:text-base text-left w-full">
                                <tr>
                                    <td>
                                        <form action="{{ route('system.password.change') }}" method="POST">
                                            @csrf
                                            @method('POST')
                                            <!-- Password Modal -->
            ...
            <div class="modal fade" id="imageModal" tabindex="-1" aria-labelledby="imageModalLabel" aria-hidden="true" data-bs-backdrop="static">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content bg-white shadow-xl font-mont">
                        ...
                        <div class="px-6 py-4">
                            <form id="image-upload-form" action="" method="POST" enctype="multipart/form-data" class="space-y-6">
                                @csrf
                                ...
                                <div class="border-t px-6 py-3 flex justify-end space-x-4">
                                    <button type="button" id="cropImageAdd" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 font-mont">Crop</button>
                                    <button type="button" id="saveImageAdd" class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 font-mont cursor-not-allowed" disabled>Save Image</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<script>
    ...
    document.getElementById('saveImageAdd').disabled = true;
    ...
    document.getElementById('cropImageAdd').addEventListener('click', function() {
        if (cropper_add) {
            ...
            canvas.toBlob(function(blob) {
                ...
                croppedBlob = blob; // Store blob for upload
                console.log('Blob created and stored:', croppedBlob);

                // Enable the Save button after cropping
                document.getElementById('saveImageAdd').disabled = false;
            }, 'image/png');
        }
    });
    ...
    document.getElementById('saveImageAdd').addEventListener('click', function() {
        if (croppedBlob) {
            ...
            console.log('Sending form data:', Array.from(formData));

            fetch('{{ route("ma.image_upload") }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
                ...
                .then(response => {
                    ...
                    // Handle redirect response (e.g., on success)
                    if (response.redirected) {
                        window.location.href = response.url;
                    } else {
                        return response.json(); // Check if the response is JSON
                    }
                })
                .then(result => {
                    console.log('Result:', result); // Log the parsed JSON result
                    if (result && result.success) {
                        swal("Changed!", "You can always edit your profile photo.", "success")
                            .then(() => {
                                location.reload();
                            });
                    } else {
                        swal("Error", "Failed to upload cropped image.", "error");
                    }
            ...
            swal("Warning", "Please crop the image before saving.", "warning");
        }
    });
</script>
