function MultiSelectTag(e, t = { shadow: !1, rounded: !0 }) {
    ...
    // Group and render resources based on resource_name (ignore serial_number)
    function f(e = null) {
        p.innerHTML = "";
        let groupedResources = {};

        // Group resources by resource_name (case-insensitive)
        for (const resource of l) {
            const resourceName = resource.label.split(" (")[0].toUpperCase();
            if (!groupedResources[resourceName]) {
                groupedResources[resourceName] = [];
            }
            groupedResources[resourceName].push(resource);
        }

        // Render the grouped resources
        for (const resourceName in groupedResources) {
            // Create group header
            ...
            // Render each resource under the group
            for (const resource of groupedResources[resourceName]) {
                resource.selected
                    ? (!b(resource.value) && C(resource), g(resource, e, !0))
                    : g(resource, e);
            }
        }
    }
        ...
        for (var e of p.children)
            e.addEventListener("click", (e) => {
                !1 === l.find((t) => t.value == e.target.dataset.value).selected
                    ? ((l.find(
                          (t) => t.value == e.target.dataset.value
                      ).selected = !0),
                      (c.value = null),
                      f(),
                      E())
    ...
    (n = document.getElementById(e)),
        (function () {
            (l = [...n.options].map((e) => ({
                ...
                (r = document.createElement("inputBody")).classList.add(
                    "input-body"
                ),
                r.append(c),
                o.append(i),
